class Substitution < ApplicationRecord
    ...
    def self.find_exchange(card_id, amount, playerClass, pos_ids, subs_ids)
        ...
        cards_for_random = Card.for_all_classes.of_rarity('Free').with_cost(exchanged.cost).or(Card.not_heroes.of_player_class(playerClass).of_rarity('Free').with_cost(exchanged.cost)).to_a.delete_if { |card| pos_ids.include?(card.id) || subs_ids.include?(card.id) }
        return [cards_for_random.sample.id, amount]
    end
end
