...
$('.class_existed').on('click', function(e) {
  e.preventDefault();
  let name = $(this).attr('class').split(' ').pop();
...
$('.costs label').click(function(e) {
  ...
  if($(this).hasClass('none')) {
    ...
    if(amount >= 30 || $(this).closest('.card').hasClass('Legendary')) {
      ...
      let lastClass = $(this).closest('.card').attr('class').split(' ').pop();
