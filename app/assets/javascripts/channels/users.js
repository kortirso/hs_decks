    ...
    if(user.length > 0) {
        App.messages = App.cable.subscriptions.create({ channel: 'UsersChannel', user_id: user.data('owner') }, {
            ...
            renderMessage: function(data) {
                return '<tr><td>' + data.check.success + '/30</td><td>' + data.check.dust + '</td><td><a href="/decks/' + data.deck.id + '">' + data.deck.name + '</a></td><td>' + data.player + '</td><td>' + data.deck.formats + '</td><td>' + data.deck.power + '</td><td>' + data.deck.author + '</td><td>' + data.username + '</td><td><a class="button small" data-turbolinks="false" href="/checks/' + data.check.id + '">' + data.button_1 + '</a></td></tr>';
            }
        });
    }
});
