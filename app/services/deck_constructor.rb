...
class DeckConstructor
  ...
  def build
    ...
    @deck = Deck.create name: { ru: deck_params[:name], en: deck_params[:name_en] }, playerClass: deck_params[:playerClass], formats: deck_params[:formats], link: deck_params[:link], caption: deck_params[:caption], caption_en: deck_params[:caption_en], author: deck_params[:author], user_id: user_id, player_id: Player.find_by_name(deck_params[:playerClass]).id, power: deck_params[:power], style_id: Style.find_by_name(deck_params[:style]).try(:id), race_id: Race.find_by_name(deck_params[:race]).try(:id), reno_type: deck_params[:reno_type] == '1' ? false : true
    build_positions
    update_deck
    true
  end
  ...
  def refresh
    return false unless good_params?
    deck.update name: { ru: deck_params[:name], en: deck_params[:name_en] }, link: deck_params[:link], caption: deck_params[:caption], caption_en: deck_params[:caption_en], author: deck_params[:author], power: deck_params[:power], style_id: Style.find_by_name(deck_params[:style]).try(:id), race_id: Race.find_by_name(deck_params[:race]).try(:id)
    update_positions
    update_deck
    true
  end
  ...
  def check_cards_class
    ids = cards_params.keys.map(&:to_i)
    allowed_cards_ids = Card.for_all_classes.or(Card.of_player_class(deck.try(:playerClass) || deck_params[:playerClass])).collect(&:id)
    errors = 0
  ...
  def update_positions
    cards_ids = cards_params.keys.map(&:to_i)
