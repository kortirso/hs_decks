...
class MulliganConstructor
  ...
  private def check_mulligan(key, cards)
  ...
  private def check_card(card_name)
    card = Card.return_by_name(card_name)
    return false unless card
    mulligan.positions.create card_id: card.id, amount: 1 unless mulligan.cards.include?(card)
  end
end
